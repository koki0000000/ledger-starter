...
| ファイル | 中身 |
|---|---|
| [`Code.gs`](Code.gs) | Gmail を読んで Notion へ書く本体（420行） |
...
確かめた: 署名の検証（正しい署名は通り、改ざんと無署名は弾く）、3秒以内の応答（実測0.185秒と0.298秒）、メニューの選択を送ると Notion の分類が実際に書き換わること。

**確かめていない:**
- 実際に Discord の画面でボタンを押すところ
- **楽天カードの確定版の記帳と、ビューカードの速報・確報の突き合わせ。**実機で Gmail と Notion を繋いだ確認は三井住友の記帳しか通していない
