...
分類が外れていたら Notion 上で選び直す。一度直した行はそのまま残る。

つなぐのは **Gmail と Notion だけ**。外部へ出ていく通信は `api.notion.com` への書き込みのみで、それ以外のサービスには何も送らない。
...
- Discord への通知と、ボタンひとつでの分類の訂正（下の画面は本体のもの）
