...
記帳のたびに Discord へ1行通知できる。Webhook URL を `Config.gs` に貼るだけ。設定しなければ通知せずに動く。

さらに Cloudflare Worker を1枚足すと（`SETUP.md` の手順7）、**通知に付いたメニューから分類をその場で直せる**。押すと Notion の行が書き換わり、Discord のメッセージも更新される。
...
- **ビューカード** — 速報は店名が「国内加盟店」。約1週間後の確報で本当の店名に書き換える。速報が届いていた利用なら、行は増えない。
...
| [`Config.gs`](Config.gs) | 値を貼るだけの設定ファイル。記帳だけなら2か所 |
| [`notion-template.md`](notion-template.md) | Notion 側に作る表の定義 |
| [`worker.js`](worker.js) | Discord の署名を検証して GAS へ中継し、GAS に代わって Discord へ投稿する（任意） |
| [`SETUP.md`](SETUP.md) | 設定手順と、動かないときの見分け方 |

## 使うには

`SETUP.md` の通りに進めれば、記帳だけなら目安30分で動く。Discord のボタンまで入れると45〜55分。
どちらもプログラミングの経験は要らず、コピーと貼り付けだけで進む。
記帳だけなら、用意するものは Gmail と Notion のアカウントだけ。Discord のボタンまで入れるなら Discord と Cloudflare のアカウントも要る。いずれも無料の範囲で、月額のサービスは使わない。
...
そこで Cloudflare Worker を1枚挟む。Worker が署名を検証し、通った中身だけを GAS へ渡す。やっているのは検証・中継・投稿の代行だけで、1ファイルに収まる。
...
- **Discord の画面でメニューを実際に操作し、Notion の分類が書き換わること**（実機で確認）

確かめていない:

- **通知そのものが Discord に出るところ。**GAS から Worker を経て Discord まで届くことは確認したが、本物の Bot トークンでの投稿が受理されるところは未確認
- 楽天カードの確定版の記帳と、ビューカードの速報・確報の突き合わせ。**実機で Gmail と Notion を繋いで通したのは三井住友の記帳だけ**

動かないところがあれば issue で教えてほしい。
